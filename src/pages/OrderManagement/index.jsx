...
import { getStageString, getSubType, getTypeString, serverType } from './map/serverType';
...
const OrderManagement = () => {
  ...
  const createModalOnFinish = async (value) => {
    const params = {
      ...
      emergency: parseInt(value.emergency, 10),
      house_id: parseInt(value.house_name, 10),
      level: 5,
      note: value.note,
    };
  ...
  const queryData = async (params, unS, unF) => {
    const queryParams = {
      page: params.current,
  ...
  const orderColumns = [
    ...
    },
    {
      ...
      dataIndex: 'type',
      valueType: 'textarea',
      renderText: (val) => getTypeString(val),
    },
    {
      title: '是否紧急',
      dataIndex: 'emergency',
    ...
    },
    {
      ...
      dataIndex: 'stage',
      valueType: 'textarea',
      renderText: (val) => getStageString(val),
    },
    {
      title: <FormattedMessage id="pages.residential.searchTable.handler" defaultMessage="操作" />,
      render: (_, entity) => {
        return (
          <>
            <a
